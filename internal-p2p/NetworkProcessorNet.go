...
func NetworkProcessorNet(w *worker.Thread, fnode *FNode) {
...
func sort(parent *worker.Thread, fnode *FNode) {
	parent.Spawn("MsgSort", func(w *worker.Thread) {
		...
		w.OnReady(func() {
			sub.Subscribe(pubsub.GetPath(fnode.NodeName, "bmv", "rest"))
		})
...
func stubs(parent *worker.Thread, fnode *FNode) {
	parent.Spawn("stubs", func(w *worker.Thread) {
		// Run init conditions. Setup publishers
		pub := pubsub.PubFactory.Base().Publish(fnode.NodeName + "/blocktime")
...
func FromPeerToPeer(parent *worker.Thread, fnode *FNode) {
	...
	s := fnode.State
	msgPub := pubsub.PubFactory.MsgSplit(100).Publish(fnode.NodeName + "/msgs")
	go msgPub.Start()
...
func BasicMessageValidation(parent *worker.Thread, fnode *FNode) {
	for i := 0; i < 2; i++ { // 2 Basic message validators
		parent.Spawn(fmt.Sprintf("BMV%d", i), func(w *worker.Thread) {
			...
			// Run init conditions. Setup publishers
			msgIn := bmv.NewBasicMessageValidator(fnode.NodeName)
...
func DataQ(node *FNode, source string, msg interfaces.IMsg) {
	q := node.State.DataMsgQueue()
	node.State.LogMessage("DataQueue", fmt.Sprintf(source+", enqueue %v", len(q)), msg)
	q <- msg
}

func sendToExecute(msg interfaces.IMsg, _ *FNode, _ string, pub pubsub.IPublisher) {
	pub.Write(msg)
}

func sortMsg(msg interfaces.IMsg, fnode *FNode, source string) {
...
func Q1(fnode *FNode, source string, msg interfaces.IMsg) {
...
func Q2(fnode *FNode, source string, msg interfaces.IMsg) {
...
func NetworkOutputs(fnode *FNode) {
	for {
...
func InvalidOutputs(fnode *FNode) {
	for {
